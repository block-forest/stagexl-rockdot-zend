...
/*	
+-----------------------------------------------------------------------------------+
| Copyright (c) 2010/11, Nils Döhring												|
| All rights reserved.																|
	...
	public function register(array $_register){
		foreach($_register as $name){
			if(!in_array($name,  $this->_supportedGlobals)){
				throw new Rockdot_Http_Exception('only supported superglobals are allowed to register', 1004);		
			}
...
/*	+-----------------------------------------------------------------------------------+
	...
	*/
	public static function setIp($ip){
		if(!filter_var($ip, FILTER_VALIDATE_IP)){ //, FILTER_FLAG_IPV4, FILTER_FLAG_IPV6
			throw new Rockdot_Http_Exception('invalid ip-address submitted', 1004);	
		}
		self::$ip = $ip;	
	}
	...
	*/
	public function getParam($name, $type = NULL){
		if($type !== NULL && !isset($this->_container[$type])){
			throw new Rockdot_Http_Exception('unsupported superglobal', 1004);	
		}
		elseif($type !== NULL && isset($this->_container[$type])){
			return(Rockdot_Data::getValueByPath($name, $this->_container[$type], $this->separator));
		}
		foreach($this->_container as $_container){
			//container == global var
			$result = Rockdot_Data::getValueByPath($name, $_container, $this->separator);
			if($result !== NULL){
				return $result;
			}
		}
	...
	*/	
	public function setParam($name, $value, $type = NULL){
		if(!is_scalar($name)){
			throw new Rockdot_Http_Exception('1st argument (name) has to be a scalar', 1004);	
		}
		if(strstr($name, $this->separator)){
			throw new Rockdot_Http_Exception('1st argument (name) contains an invalid character previous defined as easy access splitchar', 1004);	
		}
...
/*	+-----------------------------------------------------------------------------------+
	...
	*/	
	public function setSeparator($separator){
		...
		if(!in_array($separator, $_separators)){
			throw new Rockdot_Http_Exception('separator should not corrupt the parameternames. use valid separators instead: ('.implode(' ', $_separators).')', 1004);		
		}
	...
	*/	
	public static function sanitize($_input){
		if(!is_array($_input)){
			throw new Rockdot_Http_Exception('input must be type of array', 1004);		
		}
		if(!isset($_output)){
			$_output = array();
		}
	...
	*/	
	public function __set($name, $value){
		throw new Rockdot_Http_Exception('setting superglobals not allowed! use setParam() instead', 1004);
	}
	...
	*/	
	public function __unset($name){
		throw new Rockdot_Http_Exception('unset superglobal is not allowed! use deleteParam() instead', 1004);
	}	
