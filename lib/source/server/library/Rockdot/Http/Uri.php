 ...
 * @package         Rockdot_Http
 *
 * @dependencies
 * @import: Rockdot_Http_Exception	 
 */
require_once('Rockdot/Http/Exception.php');
require_once('Rockdot/Validate/Url.php');
 
class Rockdot_Http_Uri{
/*  +-----------------------------------------------------------------------------------+
	| 	class member-vars
	+-----------------------------------------------------------------------------------+  */	
	/**
...
/*	+-----------------------------------------------------------------------------------+
	...
	*/
	public function __construct($url, $_queryParams = array(), $prefix = 'Rockdot_', $separator = '&', $allowInternational = true, $allowInternal = true, $_validSchemes = NULL){
		//allow internal
	...
	* @param -> $_validSchemes 		:array (allow schemes)
	*	
	* @return :Rockdot_Http_Uri
	*/
	public static function factory($url, $_queryParams = array(), $prefix = 'Rockdot_', $separator = '&', $allowInternational = true, $allowInternal = true, $_validSchemes = NULL){
		return (
			new self(
				$url, 
				$_queryParams, 
	...
	* @access public
	*/ 
	public function setUri($url, $_queryParams = array(), $prefix = 'Rockdot_', $separator = '&'){
	...
	*/	
	private function setRewrite($doRewrite){
		if(!is_bool($doRewrite)){
			throw new Rockdot_Http_Exception('param must be type of boolean');
		}
	...
	*/	
	private function setComponents($url){
		...
		elseif( 
			...
			throw new Rockdot_Http_Exception('submitted uri($url) seems invalid. url must have protocol/scheme see supported schemes', 1002);
		}
	} 
	...
	*/	
	public static function getComponent($url, $component = NULL){
		if(!self::isValid($url)){
			throw new Rockdot_Http_Exception('submitted uri($url) seems invalid.');
		}
...
/*	+-----------------------------------------------------------------------------------+
	...
	*/
	public static function isLocal($uri){
		if(!self::isValid($uri, true, true)){
			throw new Rockdot_Http_Exception('submitted uri($url) is invalid', 1002);
		}
	...
	* @access public
	*/ 
	public static function buildURL($url, array $_queryParams = array(), $prefix = 'Rockdot_', $separator = '&', $allowInternational = true, $allowInternal = true, array $_validSchemes = array()){
		//validate input
		if(!self::isValid($url, $allowInternational, $allowInternal, $_validSchemes)){
			throw new Rockdot_Http_Exception('submitted uri($url) is invalid', 1002);
		}
		if(!self::isValidSeparator($separator)){
			throw new Rockdot_Http_Exception('submitted separator($separator) is invalid', 1003);
		}
		if(!is_string($prefix)){
			throw new Rockdot_Http_Exception('prefix must be typeof string', 1005);
		}			 		 
		//handle fragments 
		$fragment 	= strstr($url, '#');
		if($fragment !== false){
	...
	*/ 	
	public function setScheme($scheme){
		if(!$this->isValidScheme($scheme)){
			throw new Rockdot_Http_Exception('invalid scheme (not in config list)', 1005);	
		}
	...
	*/ 	
	public function setUser($userName){
		if(!is_string($userName) && $userName !== ''){
			throw new Rockdot_Http_Exception('userName must be typeof string', 1003);
		}
	...
	*/	
	public function setPass($password){
		if(!is_string($password) && $password !== ''){
			throw new Rockdot_Http_Exception('password must be typeof string', 1003);
		}
	...
	*/	
	public function setHost($host){
		if(!is_string($host)){
			throw new Rockdot_Http_Exception('host must be typeof string', 1003);
		}
	...
	*/
	public function setPort($port){
		if(!is_int($port) && $port !== '' || ($port > 65536 || $port < 0)){
			throw new Rockdot_Http_Exception('port must be typeof int or empty string', 1003);
		}
	...
	*/	
	public function setPath($path){
		if(!is_string($path)){
			throw new Rockdot_Http_Exception('path must be typeof string', 1003);
		}
	...
	*/
	public function setFragment($fragment){
		if(!is_string($fragment)){
			throw new Rockdot_Http_Exception('fragment must be typeof string', 1003);
		}
	...
	*/	
	public function setQueryString($_queryParams = array()){
		if(!is_array($_queryParams)){
			throw new Rockdot_Http_Exception('queryParams must be typeof array', 1003);
		}
	...
	*/
	public function setAllowedSchemes($_validSchemes){
		if(!is_array($_validSchemes)){
			throw new Rockdot_Http_Exception('_validSchemes must be typeof array', 1003);	
		}
	...
	*/
	public function setAllowInternal($allowInternal){
		if(!is_bool($allowInternal)){
			throw new Rockdot_Http_Exception('allowInternal must be typeof boolean', 1003);		
		}
	...
	*/
	public function setAllowInternational($allowInternational){
		if(!is_bool($allowInternational)){
			throw new Rockdot_Http_Exception('allowInternational must be typeof boolean', 1003);		
		}
	...
	*
	* @see Rockdot_Validate_Url for further information
	* 
	* @input-required:
	* @param -> $url :string
	*	
	...
	*/	
	public static function isValid($url, $allowInternational = true, $allowInternal = true, $_validSchemes = array()){
		$validate = new Rockdot_Validate_Url(
			$allowInternational, 
			$allowInternal, 
			$_validSchemes
		);
