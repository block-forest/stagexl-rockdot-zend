...
/*	
+---------------------------------------------------------------------------------------+
| Copyright (c) 2010, Nils Döhring													|
| All rights reserved.																	|
| Author: Nils Döhring <nils.doehring@gmail.com>									|
+---------------------------------------------------------------------------------------+ 
 *
 * Rockdot_Loader loads classes or files from includepath / registers autoloader 
 *
 ...
 * @package         Rockdot_Loader
 *
 * @dependencies
 * @import: Rockdot_Loader_Exception	
 */
require_once('Rockdot/Loader/Exception.php');
 
class Rockdot_Loader{	
/*	+-----------------------------------------------------------------------------------+
	| 	member vars
	+-----------------------------------------------------------------------------------+  */  	
	/**
    ...
    * @access :self
    */
	const Rockdot_LOADER_REQUIRE 		= 'require';
	const Rockdot_LOADER_REQUIRE_ONCE 	= 'require_once';
	const Rockdot_LOADER_INCLUDE 		= 'include';
	const Rockdot_LOADER_INCLUDE_ONCE 	= 'include_once';
/*	+-----------------------------------------------------------------------------------+
    ...
    */
	public static function setThrowExceptions($throwExceptions){
		if(!is_bool($throwExceptions)){
			throw new Rockdot_Loader_Exception('throwExceptions parameter must be type of boolean', 1001);		
		}
	...
	* @access public static
	*/		
	public static function load($fileOrClassName, $includePath = NULL, $typeFlag = self::Rockdot_LOADER_INCLUDE_ONCE){
		...
		foreach($fileOrClassName as $file){
			...
			elseif(self::$throwExceptions === true){
				throw new Rockdot_Loader_Exception('file: ('.$file.') not found! check includepaths: '.get_include_path(), 1002);	
			}
		}
	...
	* @access public static
	*/		
	public static function import($file, $typeFlag = self::Rockdot_LOADER_INCLUDE){
		if(!self::isFile($file) && !self::isInIncludePath($file) ){
			throw new Rockdot_Loader_Exception('the file('.$file.') you try to load does not exist', 1003); 		
		}
		switch(strtolower($typeFlag)){
			case (self::Rockdot_LOADER_INCLUDE):
				return(include($file));
			break;	
			case (self::Rockdot_LOADER_INCLUDE_ONCE):
				return(include_once($file));
			break;	
			case (self::Rockdot_LOADER_REQUIRE):
				return(require($file));
			break;	
			case (self::Rockdot_LOADER_REQUIRE_ONCE):
				return(require_once($file));
			break;	
			default:
				throw new Rockdot_Loader_Exception("import type not supported", 1004);	
			break;
		}
	}
	...
	* combined with Rockdot_Loader::auto_include
	* 
	* @input-required:
	...
	*/		
	public static function register($callback, $doPrepend = false){
		if(!is_callable($callback)){
			throw new Rockdot_Loader_Exception('invalid autoloader callback! not callable', 1001);	
		}
	...
	*/	
	private static function autoloader($class){ 
		...
		if(!self::isInIncludePath($filePath.'.php') && self::$throwExceptions === true){
			throw new Rockdot_Loader_Exception('autoload failed! could not load file! check classname: '.((string) $class).' and includepath(s): '.get_include_path(), 1001);
		}
		//include the file
	...
	*/	
	public static function addPath($_includePaths){
		...
		if(empty($_includePaths)){
			throw new Rockdot_Loader_Exception("no paths have been submitted.", 1000);
		}
	...
	*/	
	public static function removePath($_includePaths){
		...
		if(count($_currentIncludePaths) == 1){
			throw new Rockdot_Loader_Exception('cannot remove the only includepath', 1000);	
		}
	...
	*/
 	public static function isFile($path){
		if(!is_string($path)){
			throw new Rockdot_Loader_Exception("argument 1 (filepath) must be type of string", 1001);
		}
		if(is_file($path)){
			return true;
		}
