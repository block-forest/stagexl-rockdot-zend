 ...
 * @import: Rockdot_Helper_Html	
 * @import: Rockdot_Http_Uri
 * @import: Rockdot_Http_Request
 * @import: Rockdot_Http_Response	
 * @import: Rockdot_Zend_View_Helper_Exception
...
require_once('Rockdot/Http/Uri.php');
require_once('Rockdot/Helper/Html.php');
require_once('Rockdot/Zend/View/Helper/Exception.php');
 
class Rockdot_Zend_View_Helper_Page extends Zend_Controller_Plugin_Abstract{
/*	+-----------------------------------------------------------------------------------+
	| 	member vars
	+-----------------------------------------------------------------------------------+  */		
   	/**
...
/*	+-----------------------------------------------------------------------------------+
	...
	*/
	public static function setAddLanguage($addLanguage){
		if(!is_bool($addLanguage)){
			throw new Rockdot_Zend_View_Helper_Exception('addLanguage must be type of boolean');	
		}
	...
	*/
	public static function setHtmlify($htmlify){
		if(!is_bool($htmlify)){
			throw new Rockdot_Zend_View_Helper_Exception('htmlify must be type of boolean');	
		}
	...
	*/
	public function setDoctype($doctype){
		if(!is_string($doctype)){
			throw new Rockdot_Zend_View_Helper_Exception('doctype must be type of string');	
		}
	...
	*/
	public function setCharset($charset){
		if(!is_string($charset)){
			throw new Rockdot_Zend_View_Helper_Exception('charset must be type of string');	
		}
	...
	public static function link($id, array $_options = array()){
		...
		try{
			if(!is_string($id) && is_int($id)){
				throw new Rockdot_Zend_View_Helper_Exception('id must be type of string or integer', 1000);	
			}
			...
			if($node instanceof Zend_Navigation_Page_Mvc){
				...
				//--------------------------------------
				//build link
				$uri = new Rockdot_Http_Uri($link, $_options['params']);
				...
				//--------------------------------------
				//render html
				return ((string) new Rockdot_Helper_Html('a', $_options));
			}
		}
		catch(Exception $e){
			return '';
	...
	*/
    public function preDispatch(Zend_Controller_Request_Abstract $request){
        try{
			...
			if(APPLICATION_ENV === 'production' && isset($activePage->ssl) && $activePage->ssl === true && !Rockdot_Http_Request::isSSL()){
