 ...
 * @package 		Rockdot_Registry
 * 
 * @dependencies
 * @import Rockdot_Registry_Exception
 */
require_once('Rockdot/Registry/Exception.php');

class Rockdot_Registry{
/*	+-----------------------------------------------------------------------------------+
	| 	member vars
	+-----------------------------------------------------------------------------------+  */	
    /**
	...
	* @var  :string
	*/
	const DEFAULT_NAMESPACE = 'Rockdot_DEFAULT_NAMESPACE';
/*	+-----------------------------------------------------------------------------------+
	...
	*/  	
	public static function getInstance($nameSpace = NULL, array $_allowedTypes = NULL){
		...
		if(!is_string($nameSpace) || !preg_match('/[a-zA-Z_]/', $nameSpace[0])){
			throw new Rockdot_Registry_Exception('nameSpace must be type of string with a length of at least 1 char an must start with a letter', 1001);	
		}
		if(!isset(self::$_selfInstances[$nameSpace]) || self::$_selfInstances[$nameSpace] === null){
			self::$_selfInstances[$nameSpace] = new Rockdot_Registry($nameSpace, $_allowedTypes);
		}
	...
	public function set($registryId, $registerValue, $overwrite = false){
		if(is_array($this->_allowedTypes)){
			if( (is_object($registerValue) && !in_array(get_class($registerValue), $this->_allowedTypes)) ||
				...
				throw new Rockdot_Registry_Exception('this type ('.gettype($registerValue).') is not allowed. this namespace was bound to: '.implode(', ',$this->_allowedTypes), 1002);			
			}
		}
		if($this->isRegister($registryId) && $overwrite === false){
			throw new Rockdot_Registry_Exception('registry-id already in use! use overwrite-flag to re-set a register-entry.', 1003);	
		}
		if(!preg_match('/[a-zA-Z_]/', $registryId[0])){
			throw new Rockdot_Registry_Exception('a registryId must start with a letter.', 1004);		
		}
	...
	*/	
	public function __set($name, $value){
		throw new Rockdot_Registry_Exception('direct setting is not allowed! use set() instead', 1002);
	}	
