...
+---------------------------------------------------------------------------------------+ 
 *
 * @desc Rockdot_Data implements ArrayAccess, RecursiveIterator , Serializable, Countable
 * 
 ...
 * @package________Rockdot_Data
 *
 * @dependencies
 * @import: Rockdot_Data_Exception	 
 */
require_once('Rockdot/Data/Exception.php');
 
class Rockdot_Data implements ArrayAccess, RecursiveIterator, Serializable, Countable{
/*	+-----------------------------------------------------------------------------------+
	| 	member vars
	+-----------------------------------------------------------------------------------+  */
	/**
...
/*	+-----------------------------------------------------------------------------------+
    ...
    */
    public function __construct($_data = array(), $allowChanges = true){
		if(!is_array($_data)){
			throw new Rockdot_Data_Exception('1st parameter must be type of array', 1001);	
		}
	...
	*/
	final public function setAllowChanges($allowChanges){
		if(!is_bool($allowChanges)){
			throw new Rockdot_Model_Exception('allowChanges must be type of boolean', 1005);
		}
	...
	*/	
	public function setSeparator($separator){
		...
		if(!in_array($separator, $_separators)){
			throw new Rockdot_Data_Exception('separator should not corrupt the parameternames. use valid separators instead: ('.implode(' ', $_separators).')', 1004);		
		}
	...
	*/
	public static function isLastKey($key, array $_array){
		if(!is_string($key) && !is_int($key)){
			throw new Rockdot_Data_Exception('key must be type of string or integer', 1002);	
		}
	...
	*/
	public function setParam($name, $value, $overwrite = false){
		if($this->issetParam($name) && ($overwrite === false && $this->allowChanges === false)){
			throw new Rockdot_Model_Exception('set third argument overwrite to true if you want to allow overwriting parameters or use setAllowChanges() to allow allways', 1004);
		}
		if(property_exists($this, $name)){
			$this->$name = $value;	
		}
	...
	*/
	public function unsetParam($name, $overwrite = false){
		if($overwrite === false && $this->allowChanges === false){
			throw new Rockdot_Model_Exception('set 2nd argument overwrite to true if you want to allow deleting parameters or use setAllowChanges() to allow allways', 1004);
		}
