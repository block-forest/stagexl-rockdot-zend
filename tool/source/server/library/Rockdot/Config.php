...
+---------------------------------------------------------------------------------------+ 
 *
 * @desc extends Rockdot_Data for object an array-access
 ...
 * @package________Rockdot_Config
 *
 * @dependencies
 * @import: Rockdot_Config_Exception	
 * @import: Rockdot_Loader	
 * @import: Rockdot_Data
 * @import: Rockdot_Filter_ObjectToArray		 
 */
require_once('Rockdot/Config/Exception.php');
require_once('Rockdot/Loader.php');
require_once('Rockdot/Data.php');
require_once('Rockdot/Filter/ObjectToArray.php');
 
class Rockdot_Config extends Rockdot_Data{
/*	+-----------------------------------------------------------------------------------+
	| 	member vars
	+-----------------------------------------------------------------------------------+  */		
    /**
...
/*	+-----------------------------------------------------------------------------------+
	...
	*/
	public function __construct($_fileOrArray = array(), $section = NULL, $allowChanges = false){
		...
		if(!is_array($_fileOrArray) && !is_string($_fileOrArray)){
			throw new Rockdot_Config_Exception('Configuration seems invalid');	
		}
		//file based
		...
		if(!is_array($_fileOrArray)){
			throw new Rockdot_Config_Exception('Configuration is invalid', 2000);	
		}
    ...
    */
    public function toArray(){
        ...
        foreach($_data as $key => $value){
            if($value instanceof Rockdot_Data){
                $_result[$key] = $value->toArray();
            }
			elseif(is_object($value)){
				$filter = new Rockdot_Filter_ObjectToArray();
    ...
    */		
	public function set($name, $value, $overwrite = false){
		if($overwrite === true || $this->allowChanges == true){
        	...
        	throw new Rockdot_Config_Exception('Configuration-File is not writable. Modifications are not allowed. Use Flag to allow Changes');
        }
	return $this;
	}
    ...
    */		
	private function load($file){
		if(Rockdot_Loader::isFile($file) || Rockdot_Loader::isInIncludePath($file) ){
			//load by type
			$this->fileType = Rockdot_Loader::getExtension($file);
			switch($this->fileType){
				...
				break;
				default:
					throw new Rockdot_Config_Exception('config file-type not supported', 1001);
				break;
			}
		}
		else{
			throw new Rockdot_Config_Exception('config not found check passed path or|and include_path', 1002); 	
		}
		throw new Rockdot_Config_Exception('unknown error', 1003);
	}
    ...
    */
	...
	public function save($filePath = NULL, $overwrite = false){
		if($filePath !== NULL){
			...
			$this->fileType = Rockdot_Loader::getExtension($filePath);
		}
		if($this->filePath === NULL){
			throw new Rockdot_Config_Exception('no filname subitted', 1001); 	
		}
		//forbid overwrite
		if((Rockdot_Loader::isFile($this->filePath) || Rockdot_Loader::isInIncludePath($this->filePath)) && $overwrite === false){
			throw new Rockdot_Config_Exception('the file you tried to write to is locked use 1st parameter overwrite to overwrite file', 1001); 	
		}
		try{
			//open file writable 
			$fileObj = new SplFileObject($this->filePath,  "wb", (Rockdot_Loader::isFile($this->filePath) ? false : true)  );
			if($fileObj->flock(LOCK_EX)){ // exclusive lock
				...
				throw new Rockdot_Config_Exception('could not get the lock please try again', 1001);		
			}
		}
		catch(Exception $e){
			throw new Rockdot_Config_Exception('write to file failed with message('.$e->getMessage().')', $e->getCode()); 
		}
	return $this;	
	}
