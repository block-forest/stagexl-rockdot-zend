...
*/
/**
 * Rockdot_Helper_Html Helper Class for rendering Html
 * 
 * @author nils.doehring
 * @version 1.0
 * @import: Rockdot_Helper_Html_Exception
 */
require_once('Rockdot/Helper/Html/Exception.php');

class Rockdot_Helper_Html{
/*  +-----------------------------------------------------------------------------------+
	| 	class member-vars
	+-----------------------------------------------------------------------------------+  */		
	/**
...
/*	+-----------------------------------------------------------------------------------+
			...
			foreach($_input as $attribute => $parameter){
				if($parameter !== '' && is_string($parameter)){
					switch($attribute){
						...
						break;
						default:
							//throw new Rockdot_Helper_Html_Exception('Attribute: '.$attribute.' not supported');
						break;
					}
				}
			}
	...
	*/ 
	private function getClosure(){
		...
		throw new Rockdot_Helper_Html_Exception('unknown error during rendering', 1001);
	}
	...
	*/
	public function setXHtml($flag){
		if(!is_bool($flag)){
			throw new Rockdot_Helper_Html_Exception('argument (flag) must be type of boolean', 1001);
		}
		$this->isXHtml = $flag;
	return $this;
	}
	...
	*/
	private function setElement($element){
		if(!is_string($element)){
			throw new Rockdot_Helper_Html_Exception('argument (element) must be type of string', 1002);	
		}
		...
		elseif(in_array($element, $this->_validElements['selfclosing'])){	
			...
			throw new Rockdot_Helper_Html_Exception('Element: '.(string) $element.' not supported', 1003);	
		}
	...
	*
	* @input-required:
	* @param -> $data :mixed (string|Rockdot_Helper_Html)
	*
	* @return :this
	*
	* @access public
	*/
	public function addContent($data){
		if($data instanceof Rockdot_Helper_Html){
			$data = $data->render();	
		}
		$this->body .= $data;
	return $this;
