...
class Rockdot_Zend_Plugin_RewriteIp extends Zend_Controller_Plugin_Abstract{
/*	+-----------------------------------------------------------------------------------+
	| 	member vars
	+-----------------------------------------------------------------------------------+  */	
	/**
		...
		if(!empty($_proxies)){
			...
			foreach($_proxies as $lc => $ip){
				if(!preg_match('/[a-z]{2}/', $lc)){
					throw new Rockdot_Zend_Plugin_Exception('ivalid 2lc submitted', 1001);		
				}
				if(!filter_var($ip, FILTER_VALIDATE_IP)){ //, FILTER_FLAG_IPV4, FILTER_FLAG_IPV6
					throw new Rockdot_Zend_Plugin_Exception('invalid ip-address', 1001);	
				}
				$this->_proxies[$lc] = $ip;
			}
		}
	...
	*/
    public function routeStartup(Zend_Controller_Request_Abstract $request){
		...
		if(isset($request->system_iplc) && isset($this->_proxies[$request->system_iplc])){
			...
			Rockdot_Http_Request::setIp($this->_proxies[$request->system_iplc]);
