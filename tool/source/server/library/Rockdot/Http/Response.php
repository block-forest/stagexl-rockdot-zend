...
*/
/**
 * @desc Rockdot_Http_Response sends response according to request
...
/*	+-----------------------------------------------------------------------------------+
	...
	*/
	public function setStatus($statusCode){
		if((!is_int($statusCode) && !is_string($statusCode)) || !isset(self::$_responseStatus[$statusCode])){
			throw new Rockdot_Http_Exception("could not set status - invalid or unsupported status code submitted", 1001);	
		}
	...
	*/
	public function addContent($data){
		if(!is_string($data)){
			throw new Rockdot_Http_Exception('body/content must be type of string', 1001);	
		}
		$this->body .= $data;
	return $this;
	}
	...
	*/
	public static function redirect($url, $statusCode = 301){
		...
		if(!Rockdot_Http_Uri::isValid($url)){
	...
	*/
	public static function forwardSsl(){
		...
		header('Location: '.Rockdot_Http_Request::getUrl(true));
		exit();
	}
